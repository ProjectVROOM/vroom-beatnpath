...
public class SpawnNotes : MonoBehaviour
{
    ...
    {
        startTime = Time.time;
        InvokeRepeating("noteSpawn", 2f, 2.0f);
    ...
    IEnumerator noteSpawn()
    {
        ...
        //if ((Time.time - startTime) % 2.0f == 0f)
            ...
            noteLocations.Add(right.transform.position);
            noteLocations.Add(left.transform.position);  
        return null;
    }
}
